...
# fonte do CDC (ver implantacao/base-dados/).
#
# Uso principal: gerar trafego real de INSERT/UPDATE/DELETE na fonte
# Oracle para validar que o CDC do Data Movement Gateway esta de fato
# capturando mudancas incrementais (nao so o full load) — ver item de
# checklist em Guia_Instalacao_Projeto.md secao 10.
#
...
class VendasODSApp:
    ...
    def janela_confirmacao_historica(self):
        try:
            ...
            if not d_prim:
                messagebox.showwarning("Aviso", "Banco vazio. Utilize o Atualiza CDC primeiro.")
                return
    ...
    # --- 2. ATUALIZA CDC (Conforme Especificações item 3) ---
    def abrir_cdc(self):
        try:
        ...
        win = tk.Toplevel(self.root); win.title("Atualiza CDC"); win.geometry("550x350")
        win.transient(self.root); win.grab_set()
        ...
        def run():
            try:
                ...
                conn.commit()
                self.log(f"CDC {dt}: {vol} pedidos gerados.")
